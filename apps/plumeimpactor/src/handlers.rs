...
impl PlumeFrameMessageHandler {
    ...
    fn handle_message(&mut self, message: PlumeFrameMessage) { match message {
        PlumeFrameMessage::DeviceConnected(device) => {
            println!("Device connected: {}", device);
            if !self.usbmuxd_device_list.iter().any(|d| d.usbmuxd_device.device_id == device.usbmuxd_device.device_id) {
        ...
        PlumeFrameMessage::DeviceDisconnected(device_id) => {
            ...
            if let Some(index) = self.usbmuxd_device_list.iter().position(|d| d.usbmuxd_device.device_id == device_id) {
        ...
        PlumeFrameMessage::PackageSelected(package) => {
            ...
            let package_id = package.get_bundle_identifier().unwrap_or_else(|| "Unknown".to_string());
            println!("Package selected: {}", package_name);
            self.package_selected = Some(package);
            self.plume_frame.install_page.set_top_text(format!("{} - {}", package_name, package_id).as_str());
    ...
    fn usbmuxd_picker_rebuild_contents(&self) {
        ...
        for item_string in &self.usbmuxd_device_list {
            self.plume_frame.usbmuxd_picker.append(&item_string.to_string());
        }
    }
    ...
    fn usbmuxd_picker_reconcile_selection(&mut self) {
        if let Some(selected_item) = self.usbmuxd_selected_device_id.clone() {
            if let Some(new_index) = self.usbmuxd_device_list.iter().position(|d| d.usbmuxd_device.device_id.to_string() == selected_item) {
                self.plume_frame.usbmuxd_picker.set_selection(new_index as u32);
            } else {
